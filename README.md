...
工具栏「文件 → 导出配置 / 导入配置」，网页端与平板则在「⚙ 设置 → 配置备份」里。

> ⚠️ **导出时建议设置口令。** 不带口令时，访问代码是用**本机方式**加密的
> （Windows 是 DPAPI、绑定当前 Windows 用户；Linux/Docker 是本机密钥文件），
> 换一台机器或换到安卓上就**解不开**，只能重新填一遍访问代码。
> 带口令导出的文件用标准库算法加密，**在任何版本、任何平台都能导入**
> （Windows 桌面版 / Linux / Docker / 安卓通用），导入时输入同一个口令即可。

命令行（NAS / Docker 用户）：

```bash
# 导出（带口令 = 跨设备通用）
python -m app.headless --export-config ./bambu.json --config-passphrase '你的口令'
# 在另一台设备上导入
python -m app.headless --import-config ./bambu.json --config-passphrase '你的口令'
```

导入会用文件里的设备**替换**当前列表并重新连接；若某些访问代码在目标机器上解不开，
程序会**逐个点名**是哪些设备，并提示下次导出时带口令（而不是静默留空）。
...
早期版本每次只能搜到一部分，原因是三件事，现在都处理了：

* 组播只从默认路由那张网卡发出，本机有 VMware / Hyper-V / 代理网卡时，探测包常常
  没发到打印机所在网段。现在逐网卡绑定地址发包。
* 网卡列表靠主机名解析不可靠（实测漏掉 `192.168.31.x`）。现在读 Windows 适配器表，
  并取真实子网掩码。
* 固件对 SSDP 探测有节流，单次探测收不全。现在搜索窗口内每秒重发，
  每 4 秒整网段单播扫一次，结果只增不减，已保存的设备也一直列着。

实测同一网段 13 台设备：12 秒平均 8-11 台，15 秒 10-13 台，20-30 秒稳定 13/13。
...
而签名密钥只给官方应用，第三方拿不到。此时**只有灯光能控制**（灯控不受签名限制）。

要恢复暂停/停止，请在打印机屏幕上**按顺序**做这两步，**缺一不可、顺序不能反**
（改完建议**重启设备**）：

| 顺序 | 操作 | 位置 |
| --- | --- | --- |
| 1 | **先开「局域网模式」** | 设置 → 网络 → 局域网模式 / LAN Mode Only |
| 2 | **再开「开发者模式」** | 设置 → 通用 → 开发者模式 / Developer Mode |

> ⚠️ **只开「局域网模式」是不够的。** 此时画面与遥测都正常，但暂停/停止
> **仍然会被固件忽略**（现象就是按钮点了没反应）—— 很多用户卡在这一步，
> 以为软件坏了。必须**两步都开**。

**更省事的办法（推荐）**：改用拓竹官方「**农场管家 / Farm Manager**」接管这台设备。
它拿得到官方签名密钥，**不需要开发者模式**。

> ⚠️ **只在切片软件里登录拓竹账号是没用的。** 那只影响云端；
> 本项目走局域网接口，必须在打印机本机按上面的步骤放行。
>
...
界面会按命令分别置灰，并在状态条/按钮上说明哪部分被挡、怎么放行
（点一下被挡的按钮会弹出完整步骤）。
...
└─ docs/
   ...
   ├─ VERIFICATION.md            实测验证记录（真机环境、逐项结果、没验过的）
   ├─ PACKAGING.md               四种形态怎么打包，以及各平台的坑
   ├─ FIELD_NOTES.md             真机实测与调研结论（拓竹固件差异、第三方设备）
...
交付前跑过的验证，这里只列要点（完整清单与真机环境见
[`docs/VERIFICATION.md`](docs/VERIFICATION.md)）：

| 验证项 | 结果 |
| --- | --- |
| 自动搜索 | ✓ 同时发现模拟器与局域网内的 11 台真机（X1C / X2D / P1S / A1 / A1 mini / P1P） |
| 真机会话 | ✓ 13 台同时「画面 + 遥测」在线；X2D / X1C 走 RTSPS 约 8-10fps，P1/A1 走 6000 端口约 0.4-0.9fps（硬件限制） |
| 遥测准确性 | ✓ 进度、剩余时间、喷嘴/热床温度、AMS 余量、HMS 错误码全部正确 |
| TLS 证书链 | ✓ X1C / P1S / X2D 的 8883 与 6000 端口均可通过内置 Bambu Lab CA 校验 |
| 错误路径 | ✓ 访问代码错误时 1-2 秒内快速失败并准确提示，不空转 |
| 网页端 | ✓ 13 路画面走一条 `/api/live` 连接推送（绕开浏览器 6 连接限制）；无令牌 401 |
| 四种交付形态 | ✓ Windows exe / Linux 无界面 / Docker / 安卓都能起服务并出画面 |
| 配置安全 | ✓ 13 台的访问代码保存前后一致；自动备份；带口令导出可在另一平台导入 |

`selftest.bat` 与 `demo.bat` 可以自己复现（用内置模拟器，不需要真机）。

### 曾经的问题：X2D 有进度没有画面（已修复）

X2D 的 6000 端口即使访问代码完全正确也会拒绝连接，它只提供 RTSPS(322)。而早期版本
把视频通道当成一次性的：RTSPS 几秒内没出画面就永久退回 6000，于是在这类机型上
再也回不到能用的通道（叠加 X2D 证书链认证要重试，首次建连本来就更慢）。

现在按机型固定可用通道、记住可用的 TLS 参数、画面长时间无更新时自动换道重试，
RTSPS 首帧等待也从 7 秒放宽到 20 秒。RTSPS 已在 X1C / X2D 真机上验证能出画面。
