...
> 检查后确认**不是连不上 322 端口**，而是「取到码流后没人解码」，
> 而且代码在安卓上根本没去连 322。现在的做法是服务端不解码、把码流交给
> WebView 自带的解码器（WebCodecs）。

### 修复

- **安卓版（以及任何没有 OpenCV 的环境）上，只提供 RTSPS(322) 通道的机型现在有画面了**：
  X1 / X1C / X2D / H2 / P2S 以前在安卓上永远只有一句「连接中」。
  深查后确认问题**不是 322 通道连不上**（真机 322 用 Digest 鉴权正常，SDP 与
  SPS/PPS 都拿得到；MQTT 遥测一直正常，说明访问代码也没错），而是两件事：
  1. `_preferred_channel()` 在缺少 OpenCV 时把这类机型**降级到 6000 端口**，
     而它们在 6000 上一定失败 —— 322 端口从头到尾没被连过；
  2. 即使连上，服务端原来只能交「已解码的 JPEG」，而解 H.264 需要 OpenCV，
     安卓又装不了（16 KB 内存页兼容性，见 `docs/PACKAGING.md`）。
  现在新增纯标准库的 RTSP 客户端（`app/bambu/rtsp_h264.py`）：DESCRIBE(Digest) →
  SETUP → PLAY，拆交织 RTP 包（单 NAL / FU-A / STAP-A），转成 **AVCC** 交给网页端，
  由 WebView 的 `VideoDecoder` 解码后画到 `<canvas>`。**服务端一个原生库都不需要**。
  真机验证：8 秒收到 65 个访问单元（含 3 个关键帧），还原成 Annex-B 后
  65 帧全部解码成功。详见 `docs/KNOWN_ISSUES.md` §13。
- **画面出不来时不再只有「连接中」**：取不到码流、或浏览器不支持 WebCodecs
  （需要 WebView 94+）时，都会把**原因与下一步**直接画在画面区域上
  （触屏看不到悬浮提示），并说明遥测不受影响。

### 新增

- 排障脚本：`tools/rtsp_channel_check.py`（深查 322 通道）、
  `tools/rtsp_h264_check.py`（纯 Python 取流并解码自证）、
  `tools/h264_stream_check.py`（**端到端**：按网页端记录格式跑一遍再解析回来解码，
  让没有安卓设备时也能验证除「WebView 里画出来」之外的每一个环节）。

### 已知限制

- **安卓版的画面依赖 WebView 的 WebCodecs（需要 Android System WebView 94+）**。
  WebView 会自动更新，绝大多数设备都满足；过旧的设备上 322 机型仍然没有画面，
  但页面上会写明原因，且遥测（进度 / 温度 / 耗材 / HMS）完全不受影响。
- **「画面在安卓设备上真的画出来」这一步没有在真机上验证过**（本轮没有安卓设备）：
  已验证的是「取流 → 传输 → 前端拿到可解码的 H.264」（真机取样 65 个访问单元
  全部解码成功），未验证的是 WebView 里那次绘制。如果安卓上仍是黑屏，
  请把「⚙ 设置 → 通道诊断」的结果发出来。
- 首帧要等一个**关键帧**才能出画面（实测 6–8 秒的取样里能收到 1–3 个关键帧），
  所以刚点开时可能比 6000 端口的 JPEG 慢一点点。
