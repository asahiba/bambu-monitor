...
@pytest.mark.parametrize(
    ("headers", "dev_model", "name", "serial", "expected"),
    [
        # devmodel 代号既是「拓竹设备标记」，也参与机型识别
        ({"devmodel.bambu.com": "P1S"}, "P1S", "", "X", True),
        ({"devmodel.bambu.com": "N9"}, "N9", "", "X", True),  # A2L 的代号
        ({"devmodel.bambu.com": "O1C2"}, "O1C2", "", "X", True),  # H2C 双喷嘴变体
        ({"devname.bambu.com": "3DP-001"}, "", "3DP-001", "X", True),
...
def test_拓竹设备识别规则(headers, dev_model, name, serial, expected):
    """契约：优先看拓竹专有字段（devmodel 代号 / devname / devversion）、NT/ST 通知类型；

    兜底才看序列号，且长度取**区间**而非固定 15 位（实测 A2L 为 18 位），
    只要求「大写字母数字」以排除 uuid:、MAC 之类。

    注意 `dev_model` 与 `serial` 是两个不同的字段：前者是 SSDP 的
    `devmodel.bambu.com` 代号（与型号名毫无字面关系），后者是 `usn`。
    """
    assert (
        _looks_like_bambu(dict(headers), dev_model, name, serial=serial) is expected
    )
