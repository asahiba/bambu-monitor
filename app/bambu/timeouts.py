...
#: 多久没收到报文就开始「主动探活」（发一次 pushall 看有没有回应）。
#: 比 :data:`MQTT_STUCK_SECONDS` 小，这样能在用户察觉之前就确认到底卡没卡。
MQTT_PROBE_SILENCE = 45.0
#: 探活之后等回应的上限：超过还没报文就认为这条连接真的没反应了。
MQTT_PROBE_TIMEOUT = 8.0
#: 两次探活之间的最小间隔（P1 系列性能有限，官方建议 pushall 间隔不小于 5 分钟；
#: 这里是"疑似故障"时才发的探活，取得比 5 分钟短得多，但也不能太频繁）。
MQTT_PROBE_INTERVAL = 30.0
