...
class PrinterSession:
    ...
    def controls_blocked_reason(self) -> str:
        ...
        这段文案是**用户唯一的线索**，所以必须写清三件事（都有回归测试盯着）：

        1. **哪些命令真的被挡**：只有 `print` 段（暂停 / 继续 / 停止 / 速度），
           灯控走 `system` 段，不受影响 —— 也就是"此时只能控灯"；
        2. **怎么放行，以及顺序**：必须**先开局域网模式、再开开发者模式**，
           两步缺一不可。**只开局域网模式是没用的** —— 画面与遥测会正常，
           暂停/停止依旧被固件忽略（实测结论，见 docs/FIELD_NOTES.md）；
        3. **更省事的路**：改用官方「农场管家 / Farm Manager」接管设备
           （签名密钥只给官方应用，它不需要开发者模式）。

        实测依据：新机型（H2C / H2S / X2D / P2S / A2L）的 `fun` 字段会置位签名要求；
        未开开发者模式时 `print` 段命令被静默忽略，设备回 HMS 0500-0500-0001-0007。
        """
        if not self.controls_blocked:
            return ""
        return (
            "暂停 / 停止 / 速度档位这类命令被打印机固件拒绝了。\n\n"
            "原因是新固件要求 MQTT 命令签名，而签名密钥只给官方应用 —— "
            "第三方软件（包括本项目）拿不到，所以这些命令会被设备直接忽略。\n"
            "此时**只有灯光还能控制**：灯控走 system 段命令，不在签名要求范围内。\n\n"
            "要恢复暂停/停止，请在打印机屏幕上**按顺序**做这两步（缺一不可、顺序不能反）：\n"
            "  1. 先开「局域网模式」：设置 → 网络 → 局域网模式 / LAN Mode Only\n"
            "  2. 再开「开发者模式」：设置 → 通用 → 开发者模式 / Developer Mode\n"
            "两步都做完后建议重启打印机，暂停/停止才会真正生效。\n\n"
            "⚠️ 只开「局域网模式」是不够的：画面和遥测会一切正常，但暂停/停止仍然被固件忽略"
            "（现象就是按钮点了没反应）。很多人卡在这一步，以为是软件坏了。\n\n"
            "更省事的选择：改用拓竹官方「农场管家 / Farm Manager」接管这台设备 —— "
            "推荐，它拿得到官方签名密钥，不需要开发者模式。\n\n"
    ...
    def controls_blocked_short(self) -> str:
        ...
        if not self.controls_blocked:
            return ""
        return (
            "暂停/停止被固件挡住（只有灯光可用）：需先在打印机上开「局域网模式」、"
            "再开「开发者模式」；也可改用官方「农场管家 / Farm Manager」"
        )
