...
#### 三条可行的放行方式（用户侧操作，缺一不可）

第三方客户端（含本项目）只走**局域网**接口，所以：

| 方式 | 位置 | 说明 |
| --- | --- | --- |
| **局域网模式** | 设置 → 网络 → 局域网模式 / LAN Mode Only | 最常见的解法 |
| **开发者模式** | 设置 → 通用 → 开发者模式 / Developer Mode | 放行 `print` 段命令 |
| **农场管家** | 官方 Farm Manager 接管设备 | 签名密钥只给官方应用 |
...
不是软件坏了。界面提示必须按这个范围如实描述（曾经笼统写成「控制不可用」，
用户看到后去点灯发现能用，反而更困惑）。

已落入代码：`PrinterSession.command_blocked()` 按命令判定；
`controls_blocked_reason` 给出上面三条放行方式与「登录账号无效」的说明；
桌面端 `tile.py` 与网页端 `page.py` 都按命令分别置灰并显示提示。
