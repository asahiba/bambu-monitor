...
| `BAMBU_HOST` | 网页监听地址（默认 `0.0.0.0` = **监听所有网卡**；只想本机访问就设 `127.0.0.1`） | `0.0.0.0` |

> **关于默认监听所有网卡**：这是为了让同网段的手机能直接打开监控页（见 `SECURITY.md`
> 的威胁模型）。网页令牌不是身份认证，所以**不要把端口暴露到公网**；
> 只在自己电脑上看就设 `BAMBU_HOST=127.0.0.1` 或 `--host 127.0.0.1`。
